...
- **`scripts/ship-epic.sh`** — the unattended epic orchestrator (no tmux; `claude -p` per story → remote/headless-safe). Project-independent: log/state dirs derive from the repo slug, `test-gate.sh` optional, calls `/bmad-ship-story`. Install: copy to your repo's `scripts/`, run `bash scripts/ship-epic.sh --dry-run`. The 3 commit gates + party-mode failure vote are preserved. (As of 2026-06-04 the per-story RETRY path also runs a commit-integrity guard — a retry that exits 0 with uncommitted work no longer strands code into the next story.)
- **`build-factory.md`** — the outer-outer loop: a hands-off, **staged + demo-gated** factory over `ship-epic.sh`. Turns an approved spec + BMAD plan into a *usable* system through walking-skeleton → capability-milestone demos → **MVP gate (hands-on)** → build-out → retrospective → integration PR. Gates the build-out on *running, usable results* (which automated review can't judge), not just passing reviews — the antidote to "built a mountain of correct-but-unusable code, then reworked." Drop at `~/.claude/commands/build-factory.md` (user-level); `macbook-setup.sh` symlinks it.
